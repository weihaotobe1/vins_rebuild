...
PoseGraph::PoseGraph(MapDrawer* _pmapdrawer)
{
...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop, vinssystem* _pvinssystem)
{
    //shift to base frame
    printf("pg4\n");
    Vector3d vio_P_cur;
	...
	if (loop_index != -1)
	{
        ...
        if (cur_kf->findConnection(old_kf, _pvinssystem))
        {
            ...
            {  
                ...
                for(int i = 0; i < vallframes_noloop.size(); i++)
                {
                    if(vallframes_noloop[i].first == cur_kf->sequence)
                    {
                        vallframes_noloop[i].second.mP = w_r_vio * vallframes_noloop[i].second.mP + w_t_vio;
                        vallframes_noloop[i].second.mQ = w_r_vio * vallframes_noloop[i].second.mQ;
                        //cout << "w_r_vio" << endl << w_r_vio << endl;
                        //cout << "w_t_vio" << endl << w_t_vio << endl;
                    }
                }
                FILE* fpnoloop = fopen("noloop_frames.txt", "w");
                for(int i = 0; i < vallframes_noloop.size(); i++)
                {
                    fprintf(fpnoloop, "%.6lf %10.6lf %10.6lf %10.6lf\n", vallframes_noloop[i].second.header, vallframes_noloop[i].second.mP[0], vallframes_noloop[i].second.mP[1], vallframes_noloop[i].second.mP[2]);
                }
                fclose(fpnoloop);
                ...
                fclose(fploop);
                mpmapdrawer->SetAllFrames(vallframes_noloop);
    ...
    {
        printf("pg6\n");
        std::cout << VINS_RESULT_PATH << std::endl;
...
void PoseGraph::SetDrawerKeyFrames()
{
    list<KeyFrame*>::iterator it;
    vector<Vector3d> vtmpposes;
    for (it = keyframelist.begin(); it != keyframelist.end(); it++)
    {
        Vector3d tmpP;
        Matrix3d tmpR;
        (*it)->getPose(tmpP, tmpR);
        vtmpposes.push_back(tmpP);
    }
    mpmapdrawer->SetKeyFrames(vtmpposes);
}

void PoseGraph::loadKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop, vinssystem* _pvinssystem)
{
    cur_kf->index = global_index;
    global_index++;
    int loop_index = -1;
        ...
        if (cur_index != -1)
        {
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                if((*it)->has_loop)
                {
                    ...
                    if((*it)->sequence != getKeyFrame((*it)->loop_index)->sequence)
                    {
                        printf("loop edge between different sequences!\n");
                    }
            ...
            */
            m_keyframelist.lock();
            m_allframes.lock();
            vector<pair<int, MyOdometryMsg>> tmpvallframes = vallframes_noloop;
            i = 0;
            int k = 0;
            Matrix3d r_drift_it;
            Vector3d t_drift_it;
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                Vector3d P_old;
                Matrix3d R_old;
                (*it)->getVioPose(P_old, R_old);
                double tmp_yaw_drift = Utility::R2ypr(tmp_r).x() - Utility::R2ypr(R_old).x();
                r_drift_it = Utility::ypr2R(Vector3d(tmp_yaw_drift, 0, 0));
                t_drift_it = tmp_t - r_drift_it * P_old;

                while(vallframes_noloop[k].second.header <= (*it)->time_stamp )
                {
                    if(lastheader == -1)
                    {
                        k++;
                        ...
                        double dt1 = vallframes_noloop[k].second.header - lastheader;
                        double dt2 = (*it)->time_stamp - vallframes_noloop[k].second.header;
                        double dt = (*it)->time_stamp - lastheader;
                        ...
                        vallframes_noloop[k].second.mQ = r_drift_now * vallframes_noloop[k].second.mQ;
                        vallframes_noloop[k].second.mP = r_drift_now * vallframes_noloop[k].second.mP + t_drift_now;
                        k++;
                    }
                }

                lastheader = (*it)->time_stamp;
            ...
            for(;k < vallframes_noloop.size(); k++)
            {
                vallframes_noloop[k].second.mQ = r_drift * vallframes_noloop[k].second.mQ;
                vallframes_noloop[k].second.mP = r_drift * vallframes_noloop[k].second.mP + t_drift;
            }


            FILE* fploop = fopen("loop_frames.txt", "w");
            for(k = 0; k < vallframes_noloop.size(); k++)
            {
                fprintf(fploop, "%.6lf %10.6lf %10.6lf %10.6lf\n", vallframes_noloop[k].second.header, vallframes_noloop[k].second.mP[0], vallframes_noloop[k].second.mP[1], vallframes_noloop[k].second.mP[2]);
            }
            fclose(fploop);
            mpmapdrawer->SetAllFrames(vallframes_noloop);
            vallframes_noloop = tmpvallframes;
...
void PoseGraph::loadPoseGraph(vinssystem* _pvinssystem)
{
    TicToc tmp_t;
    FILE * pFile;
    string file_path = POSE_GRAPH_SAVE_PATH + "pose_graph.txt";
                                    ...
                                    &keypoints_num) != EOF) 
    {
        ...
        */
        ...
        loadKeyFrame(keyframe, 0, _pvinssystem);
        if (cnt % 20 == 0)
        {
            //publish();
        }
