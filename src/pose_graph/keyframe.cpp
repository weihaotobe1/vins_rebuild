                                      ...
                                      vector<uchar> &status)
{
    ...
    if (n >= 8)
    {
        ...
        for (int i = 0; i < (int)matched_2d_cur_norm.size(); i++)
        {
            ...
            double tmp_x, tmp_y;
            tmp_x = FOCAL_LENGTH * matched_2d_cur_norm[i].x + POSECOL / 2.0;
            tmp_y = FOCAL_LENGTH * matched_2d_cur_norm[i].y + POSEROW / 2.0;
            tmp_cur[i] = cv::Point2f(tmp_x, tmp_y);

            tmp_x = FOCAL_LENGTH * matched_2d_old_norm[i].x + POSECOL / 2.0;
            tmp_y = FOCAL_LENGTH * matched_2d_old_norm[i].y + POSEROW / 2.0;
...
bool KeyFrame::findConnection(KeyFrame* old_kf, vinssystem* _pvinssystem)
{
	TicToc tmp_t;
	...
	*/
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
	    ...
	    if (abs(relative_yaw) < 30.0 && relative_t.norm() < 20.0)
	    {
	    	...
	    	{
			    MyPointCloudMsg msg_match_points;
			    msg_match_points.header = time_stamp;
			    for (int i = 0; i < (int)matched_2d_old_norm.size(); i++)
			    {
		            Eigen::Vector3d p;
		            p[0] = matched_2d_old_norm[i].x;
		            p[1] = matched_2d_old_norm[i].y;
		            p[2] = matched_id[i];
		            msg_match_points.vp3.push_back(p);
			    }
			    Eigen::Vector3d T = old_kf->T_w_i; 
			    Eigen::Matrix3d R = old_kf->R_w_i;
			    Quaterniond Q(R);

			    msg_match_points.mP = T;
			    msg_match_points.mQ = Q;
			    msg_match_points.index = index;
				_pvinssystem->relocalization_callback(msg_match_points);
	    	}
	        return true;
	    }
	}
